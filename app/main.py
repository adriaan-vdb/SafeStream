...
# Background task for gift simulation
gift_task = None
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    global gift_task
    gift_task = await events.create_gift_task(connected)
    logging.info("Started gift simulation background task")
    ...
    if gift_task:
        gift_task.cancel()
        # Don't await the task to avoid event loop issues in tests
        logging.info("Cancelled gift simulation background task")
